...
def login_required(f):
    ...
    def wrapper(*args, **kwargs):
        # MOCK AUTH: Hilangkan pengecekan JWT token
        user = users_col.find_one({"role": "user"}) or users_col.find_one()
        if user:
...
def admin_required(f):
    ...
    def wrapper(*args, **kwargs):
        # MOCK AUTH: Hilangkan pengecekan JWT token
        admin = users_col.find_one({"role": "admin"}) or users_col.find_one()
        if admin:
            g.user_id = str(admin["_id"])
        else:
            g.user_id = str(ObjectId())
        g.role = "admin"
        return f(*args, **kwargs)
...
def login():
    # MOCK AUTH: Selalu sukses, agar Locust dapat token dan tidak menskip request
    token = make_token(str(ObjectId()), "user")
    return jsonify({
        "token": token,
        "user": {
            "id":    str(ObjectId()),
...
# ═══════════════════════════════════════════
# Simple Endpoints (untuk index.html versi lama)
# ═══════════════════════════════════════════

@app.route("/order", methods=["POST"])
def simple_create_order():
    d = request.get_json() or {}
    import uuid
    doc = {
        "order_id": str(uuid.uuid4()),
        "customer_name": "Simple User",
        "total": float(d.get("price", 0)) * int(d.get("quantity", 1)),
        "status": "pending",
        "created_at": now_iso(),
        "items": [{
...
def simple_update_order(order_id):
    d = request.get_json() or {}
